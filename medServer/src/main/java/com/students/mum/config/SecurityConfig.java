...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		http.addFilter(authen).addFilter(new JwtAuthorizationFilter(authenticationManager())).authorizeRequests()
				.antMatchers("/login", "/registration", "/h2-console/**").permitAll().anyRequest().authenticated().and()
				.formLogin().loginPage("/login").failureUrl(LOGIN_ERROR).defaultSuccessUrl("/").and().logout()
				.deleteCookies(SecurityConstants.TOKEN_COOKIE_NAME).and().csrf().disable()// .ignoringAntMatchers("/h2-console/**").and()//
																							// .ignoringAntMatchers("/h2-console/**");//
				.sessionManagement().sessionCreationPolicy(SessionCreationPolicy.STATELESS).and() // .disable();//
				// // don't apply CSRF
				// protection to /h2-console
				.exceptionHandling().accessDeniedPage("/access-denied").and().rememberMe()
